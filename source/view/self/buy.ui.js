...
do_ALayout_buy.on("touch", "", 2000, function() {
	...
	var sqls = [];
	var d=new Date();
	var ymd=d.getFullYear()+"-"+(d.getMonth()+1)+"-"+d.getDate()+ " " +d.getHours()+":"+d.getMinutes()+":"+d.getSeconds();
	ymd=ymd.replace(/\b(\w)\b/g, "0$1");
	
	sqls.push("insert into Buy (phone,BarCodeID,price,num,datetime) select phone,BarCodeID,price,num,'" + ymd + "' FROM BuyCar where phone='" + phone + "'");
	sqls.push("delete from BuyCar where phone='" + phone + "'");

	do_SQLite.execute1(sqls,true,function(data,e){
		do_SQLite.close();
		do_Notification.toast("已经购买成功");
		do_App.closePage("refreshBuyNum");
	});
